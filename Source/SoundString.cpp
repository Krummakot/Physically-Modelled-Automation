...
SoundString::SoundString(NamedValueSet& parameters, double sampleRate) :
...
void SoundString::processScheme() {
...
double SoundString::getOutput(double outPos) {
    return uPtr[1][static_cast<int>(round(N+1) * outPos)];
}

void SoundString::setPluckLoc(double pluckLoc) {
    this->pluckLoc = (round(N+1) * pluckLoc);
}

void SoundString::setOutLoc(double outLoc) {
    this->outPos = outLoc;
}

void SoundString::setDamping(double s0, double s1) {
    this-> s0 = s0;
    this-> s1 = s1;
}

void SoundString::setE(double newE) {
    this-> E = newE;
}

void SoundString::setTension(double newTension) {
    this-> tension = newTension;
}

void SoundString::setStringLength(double newLength) {
    this-> stringLength = newLength;
}

void SoundString::updateCoefficients() {
    c = sqrt(tension/(p*A));
    h = sqrt((c * c * k * k + 4.0 * s1 * k
             + sqrt (pow (c * c * k * k + 4.0 * s1 * k, 2.0)
                    + 16.0 * kappa * kappa * k * k)) * 0.5);
...
void SoundString::excite(int exciteSelection, double velocity, int newWidth) {
    width = newWidth;
